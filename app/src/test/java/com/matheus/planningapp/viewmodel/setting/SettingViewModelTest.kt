...
class SettingViewModelTest {
    ...
    @Test
    fun `uiState should emit initial state with default settings from repositories`() = runTest {
        // Given
    ...
    @Test
    fun `uiState should emit updated state when settings flow changes`() = runTest {
        // Given
        val collectedStates = mutableListOf<SettingUiState>()
    ...
    @Test
    fun `updateSettings should save settings to repository when only view mode changes`() = runTest {
        // Given
    ...
    @Test
    fun `updateSettings should save settings when notification option changes`() = runTest {
        // Given
    ...
    @Test
    fun `setNotificationToFutureCommitment should schedule notifications for all future commitments`() = runTest {
        // Given
        val futureCommitments = listOf(
            CommitmentEntity(
                id = 1,
                calendar = 1,
                title = "Future Task 1",
    ...
    @Test
    fun `setNotificationToFutureCommitment should handle empty commitment list`() = runTest {
        // Given
        coEvery { commitmentRepository.getFutureCommitments() } returns emptyList()
        coEvery { taskNotificationScheduler.scheduleTaskNotification(any()) } just Runs
    ...
    @Test
    fun `deleteNotificationToFutureCommitments should cancel notifications for all future commitments`() = runTest {
        // Given
        val futureCommitments = listOf(
            CommitmentEntity(
                id = 1,
    ...
    @Test
    fun `deleteNotificationToFutureCommitments should handle empty commitment list`() = runTest {
        // Given
        coEvery { commitmentRepository.getFutureCommitments() } returns emptyList()
        coEvery { taskNotificationScheduler.cancelTaskNotification(any()) } just Runs
    ...
    @Test
    fun `updateSettings should preserve current view mode when only notification changes`() = runTest {
        // Given
        coEvery { settingsRepository.viewModeFlow } returns flowOf(ViewEnum.GRID)
        coEvery { settingsRepository.notificationOptionFlow } returns flowOf(NotificationEnum.NO_SEND)
    ...
    @Test
    fun `updateSettings should handle switching between different notification options`() = runTest {
        // Given
        coEvery { settingsRepository.viewModeFlow } returns flowOf(ViewEnum.COLUMN)
        coEvery { settingsRepository.notificationOptionFlow } returns flowOf(NotificationEnum.ALL_COMMITMENT)
...
//        coVerify(atLeast = 1) { taskNotificationScheduler.cancelTaskNotification(any()) }
    ...
    @Test
    fun `updateSettings should not process notifications if only view mode changes`() = runTest {
        // Given
        coEvery { settingsRepository.viewModeFlow } returns flowOf(ViewEnum.COLUMN)
        coEvery { settingsRepository.notificationOptionFlow } returns flowOf(NotificationEnum.NO_SEND)
