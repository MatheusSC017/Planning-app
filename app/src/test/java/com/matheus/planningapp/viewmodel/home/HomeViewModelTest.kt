...
class HomeViewModelTest {
    ...
    @Test
    fun `onSelectedDate with valid date should update selected date`() = runTest {
        // Given
    ...
    @Test
    fun `onSelectedDate with null values should keep current values`() = runTest {
        // Given
        val collectedStates = mutableListOf<HomeUiState>()
        val collectJob = launch {
            viewModel.uiState.collect { state ->
    ...
    @Test
    fun `onSelectedDate should coerce day to valid range for month`() = runTest {
    ...
    @Test
    fun `onSelectedDate with year only should update year and keep month and day`() = runTest {
        // Given
        val collectedStates = mutableListOf<HomeUiState>()
    ...
    @Test
    fun `incrementYear should increase year by one`() = runTest {
        // Given
        val collectedStates = mutableListOf<HomeUiState>()
    ...
    @Test
    fun `decrementYear should decrease year by one`() = runTest {
        // Given
        val collectedStates = mutableListOf<HomeUiState>()
    ...
    @Test
    fun `getCommitmentsForDay should combine daily and recurrence commitments`() = runTest {
        // Given
        val dayStart = Clock.System.now()
        val dayEnd = Clock.System.now().plus(kotlin.time.Duration.parse("1d"))
        val calendarId = 1L

        val dailyCommitments = listOf(
            CommitmentEntity(
                id = 1,
                calendar = calendarId,
        ...
        val recurrenceCommitments = listOf(
            CommitmentEntity(
                id = 2,
                calendar = calendarId,
                title = "Daily Standup",
    ...
    @Test
    fun `getCommitmentsForDay should return empty list when no commitments exist`() = runTest {
        // Given
        val dayStart = Clock.System.now()
    ...
    @Test
    fun `deleteCommitment should call repository deleteCommitment`() = runTest {
        // Given
        val commitmentToDelete = CommitmentEntity(
            id = 1,
            calendar = 1,
    ...
    @Test
    fun `init should ensure default calendar exists on creation`() = runTest {
        // Given
        coEvery { calendarRepository.ensureDefaultCalendarExists() } returns Unit
